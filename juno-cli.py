...
def CLI_Chat(prompt_input):
    previous_msgs = []
    stream = Juno.chat_stream(prompt_input, previous_msgs)                  # get a stream given the prompt
    final_output = ""                                                       # We need a variable to hold the message
    print("\033[1A", end="\033[34m", flush=True)                            # Set color to blue
    for chunk in stream:                                                    # The stream is an iterator, so we can loop through it to get the output in chunks
